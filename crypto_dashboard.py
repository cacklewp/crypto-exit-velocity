...
# Extract numeric value and classification from F&G string
def extract_fng_parts(fng_str):
    if isinstance(fng_str, str):
        match = re.search(r'(\d+)', fng_str)
        value = int(match.group(1)) if match else 0
        classification = fng_str.split('(')[1].rstrip(')') if '(' in fng_str else "Unknown"
        return value, classification
    return int(fng_str), "Unknown"

btc_val, btc_class = extract_fng_parts(global_fng)
eth_val, eth_class = extract_fng_parts(eth_fng)
sol_val, sol_class = extract_fng_parts(sol_fng)

# BitcoinFear-style half-circle dial (shows number in center)
...
with tab1:
    ...
    with c3:
        st.plotly_chart(fng_dial(btc_val, "F&G (Global)"), use_container_width=True)
        st.caption(f"Classification: {btc_class}")
...
with tab2:
    ...
    with c2:
        ...
        st.markdown("<p style='text-align:center; font-size:18px; color:#555; margin-top:-10px;'>Composite Velocity</p>", unsafe_allow_html=True)
    with c3:
        st.plotly_chart(fng_dial(eth_val, "F&G (ETH-specific)"), use_container_width=True)
        st.caption(f"Classification: {eth_class}")
...
with tab3:
    ...
    with c2:
        ...
        st.markdown("<p style='text-align:center; font-size:18px; color:#555; margin-top:-10px;'>Composite Velocity</p>", unsafe_allow_html=True)
    with c3:
        st.plotly_chart(fng_dial(sol_val, "F&G (SOL-specific)"), use_container_width=True)
        st.caption(f"Classification: {sol_class}")
